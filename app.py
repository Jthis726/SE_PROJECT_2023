...
template = {
    ...
    'remain_coin' : 100,            # 마켓 기본 코인
    'bank_money' : 100000000        # 은행 보유 금액
}

#init data -> 매번 회원가입하기 번거로우므로 미리 초기화
...
def fakebank():
    if request.method == 'POST':
        ...
        bm = template['bank_money']

        if bm >= amount:
            User.add_money(amount)
            bm = bm - int(amount)
            template['bank_money'] = bm
...
def Upload():
    if request.method == 'POST':
        ...
        return redirect('/')
    id = Product.add_product(Coin, Price, template['user'].name)
...
def product_buy(product_id):
    Price, Coin = Product.buy(product_id);

    template['all_Products'] = Product.product_list
    User.sub_money(Price*Coin)
    User.add_coin(Coin)
...
@APP.route('/chart')
def chart():
    return render_template('graph.html', template=template)
